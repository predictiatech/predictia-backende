...
function buildTopEvsFromSnapshot(snapshot, oddMin, oddMax) {
  ...
  // ✅ odds já estão no range em snapshot.odds, mas garantimos o filtro por segurança
  const filteredCatalog = oddsCatalog.filter((o) => {
    const odd = safeNumber(o?.odd, 0);
    return odd >= oddMin && odd <= oddMax;
  });

  // ✅ computeTopEvsForCatalog NÃO ALTERADO
  const top = computeTopEvsForCatalog(filteredCatalog, bms, EV_TOP_N, EV_MIN_PCT);

  // ✅ garante no máximo 6 (Top 6)
...
async function getAIAnalysisFromSnapshot(snapshot, cacheKey = "") {
  ...
  if (!topEvs) {
    // ✅ fluxo: filtra odds 1.40-2.30 -> EV engine -> só TOP 6 vão pra IA
    topEvs = buildTopEvsFromSnapshot(snapshot, oddMin, oddMax);
    evCacheSet(evKey, topEvs);
  }

  // ✅ (2) Enviar para IA apenas TOP 6 EVs + momentum
  const aiData = {
    match,
    live: {
